            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="item in fraternidades" :key="item.idFraternidad" class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                  ...
                  >
                    {{ item.estadoEvaluacion === 'COMPLETADO' ? 'Nota Sellada' : (item.estadoEvaluacion === 'EN_PROGRESO' ? 'Continuar Eval.' : 'Iniciar Eval.') }}
                    <span v-if="item.estadoEvaluacion !== 'COMPLETADO'" class="material-symbols-outlined text-[16px]">
...
const cargarFaseData = async () => {
  ...
  try {
    ...
    iniciarCronometro(new Date(data.fase.fechaFin))
    lanzarModalBienvenida(data.fase)
  } catch (err) {
...
const lanzarModalBienvenida = (faseInfo) => {
  ...
  if (!sessionStorage.getItem(key)) {
    const inicioFmt = new Date(faseInfo.fechaInicio).toLocaleDateString()
    const finFmt = new Date(faseInfo.fechaFin).toLocaleDateString()
