  ...
  <div class="dashboard-page max-w-3xl">
    <div class="mb-8 sm:mb-10 text-left">
      <div class="flex items-center gap-3 mb-2">
        ...
        <h2 class="dashboard-page-title italic uppercase text-primary">Nómina Excel</h2>
      </div>
      <p class="text-slate-500 font-medium">
        Sube el archivo Excel con la nómina de integrantes de tu fraternidad.
        Solo puede existir <strong>un archivo</strong> por fraternidad; al subir otro se reemplaza.
      </p>
    </div>
    ...
    <div v-else-if="!fraternidad" class="bg-amber-50 border border-amber-200 rounded-2xl p-8 text-center">
      ...
      <p class="text-amber-700 text-sm font-medium">
        Debes tener una fraternidad vinculada antes de subir la nómina.
      </p>
    </div>
    ...
    <template v-else>
      <div class="bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden mb-6">
        ...
        <div class="p-6 sm:p-8">
          <div v-if="lista" class="mb-8 p-5 bg-emerald-50 border border-emerald-100 rounded-xl">
            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="font-bold text-slate-800 truncate" :title="lista.nombreOriginal">
              ...
              <div class="flex flex-wrap gap-2 shrink-0">
                <button
                  type="button"
                  @click="descargar"
                  class="px-4 py-2 bg-primary text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-blue-900 transition-colors"
                >
                  Descargar
                </button>
                <button
                  type="button"
                  @click="eliminar"
                  class="px-4 py-2 bg-white border border-red-200 text-red-700 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-red-50 transition-colors"
                >
                  Eliminar
                </button>
              </div>
          ...
          >
            ...
            <p class="font-black text-slate-700 uppercase text-sm mb-1">
              {{ lista ? 'Reemplazar nómina' : 'Subir nómina' }}
            </p>
            <p class="text-slate-400 text-xs mb-4">Solo .xlsx o .xls, máximo 15 MB</p>
          ...
          <div class="mt-6 flex justify-end">
            ...
            >
              ...
              {{ uploading ? 'Subiendo...' : (lista ? 'Actualizar nómina' : 'Guardar nómina') }}
            </button>
          </div>
        </div>
      </div>
...
const setArchivo = (file) => {
  ...
  if (!name.endsWith('.xlsx') && !name.endsWith('.xls')) {
    notify.warning('Formato inválido', 'Solo se permiten archivos Excel (.xlsx o .xls).')
    return
  }
...
const subir = async () => {
  ...
  try {
    ...
    notify.success('Guardado', 'La nómina Excel se registró correctamente.')
  } catch (error) {
    notify.error('Error', error.response?.data?.message || 'No se pudo subir el archivo.')
  } finally {
...
const eliminar = async () => {
  const conf = await notify.confirm(
    '¿Eliminar nómina?',
    'Se quitará el Excel de tu fraternidad. Podrás subir otro después.',
    'Sí, eliminar',
  )
