          ...
          >
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-1 shrink-0">
                <button
                  @click.stop="abrirModalFacultad(fac)"
                  title="Editar facultad"
                  class="size-8 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-primary hover:border-primary/40 flex items-center justify-center transition-colors"
                >
                  <span class="material-symbols-outlined text-[18px]">edit</span>
                </button>
                <button
                  @click.stop="eliminarFacultad(fac)"
                  title="Eliminar facultad"
                  class="size-8 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-secondary hover:border-secondary/40 flex items-center justify-center transition-colors"
                >
    ...
    <div class="mt-8 bg-white rounded-3xl border-2 border-dashed border-slate-200 p-8 flex flex-col items-center">
        ...
        <div class="flex flex-wrap justify-center gap-3">
            <div
              ...
              :key="inst.idInstitucionExterna"
              class="px-4 py-2 bg-slate-100 rounded-xl flex items-center gap-2 border border-transparent hover:border-slate-300 transition-all"
            >
              <div class="flex flex-col min-w-0">
                <span v-if="inst.sigla" class="text-[9px] font-black text-primary uppercase leading-tight">{{ inst.sigla }}</span>
                <span class="font-bold text-slate-600 text-sm italic leading-tight truncate">{{ inst.nombre }}</span>
              </div>
              <div class="flex gap-1 shrink-0 ml-1">
                <button
                  @click="abrirModalInstitucion(inst)"
                  title="Editar institución"
                  class="size-7 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-primary hover:border-primary/40 flex items-center justify-center transition-colors"
                >
                  <span class="material-symbols-outlined text-[16px]">edit</span>
                </button>
                <button
                  @click="eliminarInstitucion(inst)"
                  title="Eliminar institución"
                  class="size-7 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-secondary hover:border-secondary/40 flex items-center justify-center transition-colors"
                >
...
const eliminarCarrera = async (car) => {
    const res = await Swal.fire({
        title: '¿Eliminar carrera?',
        text: `"${car.nombre}" se eliminará de forma permanente.`,
        icon: 'warning',
    ...
    try {
        ...
        seleccionarFacultad(facultadSeleccionada.value)
        Swal.fire({ icon: 'success', title: 'Carrera eliminada', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 })
    } catch (e) {
...
const eliminarInstitucion = async (inst) => {
    const res = await Swal.fire({
        title: '¿Eliminar institución?',
        text: `"${inst.nombre}" dejará de estar disponible en el sistema.`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        confirmButtonText: 'Sí, eliminar',
    ...
    try {
        ...
        cargarDatos()
        Swal.fire({ icon: 'success', title: 'Institución eliminada', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 })
    } catch (e) {
