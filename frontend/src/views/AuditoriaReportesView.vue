  ...
  <div class="dashboard-page max-w-6xl w-full">
    ...
    <div class="flex flex-wrap gap-2 mb-6 border-b border-slate-200">
      ...
      <button
        ...
        class="pb-3 px-3 text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all relative"
        :class="activeTab === 'costos' ? 'text-primary' : 'text-slate-400 hover:text-slate-600'"
      >
        Informe de Costos
        <div v-if="activeTab === 'costos'" class="absolute bottom-0 left-0 w-full h-0.5 bg-secondary rounded-t-full"></div>
      </button>
    </div>
...
import InformeCostosView from './InformeCostosView.vue'
